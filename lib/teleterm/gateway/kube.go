...
func (g *Gateway) makeKubeConfig(key *keys.PrivateKey, cas map[string]tls.Certificate) error {
	// TODO do better
	kubeConfigPath := g.cfg.CertPath + ".kubeconfig"
	...
	values := &kubeconfig.LocalProxyValues{
		...
		Clusters: []kubeconfig.LocalProxyCluster{{
			...
			// TODO support for Impersonate, ImpersonateGroups, Namespace
		}},
	...
	return trace.Wrap(kubeconfig.Save(kubeConfigPath, *config))
}
