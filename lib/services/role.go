...
func (set RoleSet) EnumerateDatabaseUsers(database types.Database, extraUsers ...string) EnumerationResult {
	...
	for _, user := range users {
		err := set.checkAccess(database, AccessMFAParams{Verified: true}, NewDatabaseUserMatcher(database, user))
...
// DELETE IN 10.0.0
func DowngradeRoleToV4(r *types.RoleImpl) (*types.RoleImpl, error) {
	switch r.Version {
	case types.V3, types.V4:
		return r, nil
	case types.V5:
		var downgraded types.RoleImpl
		downgraded = *r
