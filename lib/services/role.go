...
func (a *accessChecker) EnumerateEntities(resource AccessCheckable, listFn roleEntitiesListFn, newMatcher roleMatcherFactoryFn, extraEntities ...string) EnumerationResult {
	...
	for _, role := range a.RoleSet {
		...
		if err := a.checkRoleBasedAccess(RoleSet{role}, resource, AccessState{MFAVerified: true}); err == nil {
...
func (a *accessChecker) GetAllowedLoginsForResource(resource AccessCheckable) ([]string, error) {
	...
	for _, role := range a.RoleSet {
		...
		for _, login := range loginGetter(types.Allow) {
			// If it's false, don't reset to true. If it's already true, no need
			// to set again.
...
func (a *accessChecker) CheckDatabaseRoles(database types.Database) (create bool, roles []string, err error) {
	...
	for _, role := range autoCreateRoles {
		match, _, err := checkRoleLabelsMatch(types.Allow, role, database, a.info.Traits)
		if err != nil {
	...
	for _, role := range autoCreateRoles {
		match, _, err := checkRoleLabelsMatch(types.Deny, role, database, a.info.Traits)
		if err != nil {
			return false, nil, trace.Wrap(err)
		}
		if !match {
...
outer:
	...
	var errs []error
	for _, role := range a.RoleSet {
		matchLabels, labelsMessage, err := checkRoleLabelsMatch(types.Deny, role, rc, a.info.Traits)
		if err != nil {
			return trace.Wrap(err)
		}
		if matchLabels {
	...
	for _, role := range a.RoleSet {
		matchLabels, labelsMessage, err := checkRoleLabelsMatch(types.Allow, role, rc, a.info.Traits)
		if err != nil {
			return trace.Wrap(err)
		}
...
func (l *kubernetesClusterLabelMatcher) Match(role types.Role, typ types.RoleConditionType) (bool, error) {
	...
	ok, _, err := checkLabelsMatch(typ, labelMatchers, mapLabelGetter(l.clusterLabels), l.userTraits)
...
func (a *accessChecker) checkRoleBasedAccess(roles RoleSet, r AccessCheckable, state AccessState, matchers ...RoleMatcher) error {
	// Note: logging in this function only happens in debug mode. This is because
	...
	for _, role := range roles {
		...
		matchLabels, labelsMessage, err := checkRoleLabelsMatch(types.Deny, role, r, a.info.Traits)
		if err != nil {
			return trace.Wrap(err)
		}
		if matchLabels {
	...
	for _, role := range roles {
		...
		matchLabels, labelsMessage, err := checkRoleLabelsMatch(types.Allow, role, r, a.info.Traits)
		if err != nil {
			return trace.Wrap(err)
		}
...
func checkRoleLabelsMatch(
	...
	return checkLabelsMatch(condition, labelMatchers, resource, userTraits)
}
...
func (a *accessChecker) DesktopGroups(s types.WindowsDesktop) ([]string, error) {
	groups := make(map[string]struct{})
	for _, role := range a.RoleSet {
		result, _, err := checkRoleLabelsMatch(types.Allow, role, s, a.info.Traits)
		if err != nil {
			return nil, trace.Wrap(err)
		}
	...
	for _, role := range a.RoleSet {
		result, _, err := checkRoleLabelsMatch(types.Deny, role, s, a.info.Traits)
		if err != nil {
			return nil, trace.Wrap(err)
		}
		if !result {
...
func (a *accessChecker) getKubeResources(cluster types.KubeCluster) (allowed, denied []types.KubernetesResource) {
	for _, role := range a.RoleSet {
		matchLabels, _, err := checkRoleLabelsMatch(types.Allow, role, cluster, a.info.Traits)
	...
	for _, role := range a.RoleSet {
		matchLabels, _, err := checkRoleLabelsMatch(types.Deny, role, cluster, a.info.Traits)
		if err != nil || !matchLabels {
			continue
		}
