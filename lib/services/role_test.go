...
func TestGuessIfAccessIsPossible(t *testing.T) {
	...
	ownSessions, err := types.NewRoleV3("own-sessions", types.RoleImplSpec{
		Allow: types.RoleConditions{
			Rules: []types.Rule{
				{
					Resources: []string{types.KindSession},
	...
	require.NoError(t, err)
	ownSSHSessions, err := types.NewRoleV3("own-ssh-sessions", types.RoleImplSpec{
		Allow: types.RoleConditions{
			Rules: []types.Rule{
				{
	...
	readAllSessions, err := types.NewRoleV3("all-sessions", types.RoleImplSpec{
		Allow: types.RoleConditions{
			Rules: []types.Rule{
				{
					Resources: []string{types.KindSession},
	...
	require.NoError(t, err)
	allowSSHSessions, err := types.NewRoleV3("all-ssh-sessions", types.RoleImplSpec{
		Allow: types.RoleConditions{
			Rules: []types.Rule{
				{
					Resources: []string{types.KindSSHSession},
	...
	require.NoError(t, err)
	denySSHSessions, err := types.NewRoleV3("deny-ssh-sessions", types.RoleImplSpec{
		Deny: types.RoleConditions{
			Rules: []types.Rule{
				{
					Resources: []string{types.KindSSHSession},
...
func TestCheckAccessToDatabaseUser(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			for _, access := range tc.access {
				err := tc.roles.checkAccess(access.server, AccessMFAParams{}, NewDatabaseUserMatcher(access.server, access.dbUser))
				if access.access {
					require.NoError(t, err)
				} else {
					require.Error(t, err)
...
func TestRoleSetLockingMode(t *testing.T) {
	...
	newRoleWithLockingMode := func(t *testing.T, mode constants.LockingMode) types.Role {
		role, err := types.NewRoleV3(uuid.New().String(), types.RoleImplSpec{Options: types.RoleOptions{Lock: mode}})
		require.NoError(t, err)
		return role
	}
...
func TestExtractConditionForIdentifier(t *testing.T) {
	...
	allowWhere := func(where string) types.Role {
		role, err := types.NewRoleV3(uuid.New().String(), types.RoleImplSpec{Allow: types.RoleConditions{
	...
	denyWhere := func(where string) types.Role {
		role, err := types.NewRoleV3(uuid.New().String(), types.RoleImplSpec{Deny: types.RoleConditions{
			Rules: []types.Rule{{Resources: []string{types.KindSession}, Verbs: []string{types.VerbList}, Where: where}},
		}})
		require.NoError(t, err)
		return role
