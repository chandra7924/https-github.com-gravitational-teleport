...
func TestGetAllowedKubeUsersAndGroupsForCluster(t *testing.T) {
	...
	roleWithMultipleLabels := &types.RoleImpl{
		Spec: types.RoleImplSpec{
			Allow: types.RoleConditions{
	...
	tt := []struct {
		...
		},
		{
			...
			roleSet: services.NewRoleSet(&types.RoleImpl{
				Spec: types.RoleImplSpec{
					Allow: types.RoleConditions{
		...
		},
		{
			...
			roleSet: services.NewRoleSet(devEnvRole, &types.RoleImpl{
				Spec: types.RoleImplSpec{
