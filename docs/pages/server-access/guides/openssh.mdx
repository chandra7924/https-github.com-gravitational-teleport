...
On the machine the cluster's Proxy is running on, use the `tctl` tool to generate a new token. In the
...
resource on your cluster with the command:

```code
$ teleport join openssh --address <NODE-ADDRESS> --proxy-server <PROXY-ADDRESS> --join-method token --token <TOKEN>
```
